...
export function LawsPage(): JSX.Element {
  ...
  return <div className="page-stack">
    <PageHeader eyebrow="LIBRARY / 法規資料庫" title="法規管理" description="先建立法規，再匯入已校對的法條原文。系統不內建或推測正式法條內容。" actions={<Button onClick={() => setShowCreate(true)}>＋ 建立法規</Button>} />
...
function LawCard({ law, onEdit, onDelete }: { law: LawCollection; onEdit: () => void; onDelete: () => void }): JSX.Element {
  ...
  return <article className="law-card card"><div className="law-card-head"><div className="law-symbol">§</div><div className="law-card-title"><div><h2>{law.name}</h2><span>{law.shortName} · {law.category}</span></div>{law.examScope && <span className="scope-tag">考試範圍</span>}</div></div><div className="law-meta"><span>建立於 {formatDateTW(law.createdAt)}</span><Stars value={law.importance} /></div><ProgressBar value={average} label="整體熟練度" tone={average >= 80 ? 'green' : 'blue'} /><div className="law-card-stats"><div><strong>{articles.length}</strong><span>法條</span></div><div><strong>{mastered}</strong><span>已精通</span></div><div><strong>{Math.round(average)}%</strong><span>平均</span></div></div>{law.notes && <p className="law-note">{law.notes}</p>}<div className="card-actions"><Link className="button button-primary" to={`/articles?law=${law.id}`}>管理法條</Link><Button variant="ghost" onClick={onEdit}>編輯</Button><Button variant="ghost" onClick={onDelete}>封存</Button></div></article>
}
